...
- Реализуйте методы `save, get, delete, clear, getAll, size` в классе `com.webapp.storage.com.webapp.storage`, организовав хранение резюме в массиве 
...
1. В методе `clear()` обнуление массива предполагает обнуление (null) ячеек, где хранятся com.webapp.model.com.webapp.model, а не создание нового или присваивание ему null
1. При реализации методов не используйте коллекции
1. Не меняйте сигнатуры методов в `com.webapp.storage.com.webapp.storage`
1. Не добавляйте в `com.webapp.model.com.webapp.model` новые поля
1. com.webapp.model.com.webapp.model r — давайте переменным осмысленные имена, например resume. r допустимо в коротких циклах и лямбда-выражениях
